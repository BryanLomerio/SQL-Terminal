export const convertMySQLtoSQLite = (query) => {
    ...
    query = query.replace(/RIGHT\s+JOIN\s+projects\s+p\s+ON\s+1\s*=\s*1/gi, "");
    query = query.replace(/\s+WITH\s+ROLLUP/gi, "");
    query = query.replace(/GROUP_CONCAT\(([^()]+?)\s+ORDER\s+BY\s+[^()]+\s+SEPARATOR\s+(['"])([^'"]+?)\2\s*\)/gi, (match, expr, quote, sep) => {
        expr = expr.trim();
    ...
    query = query.replace(/\bAUTO_INCREMENT\b/gi, "AUTOINCREMENT");
    query = query.replace(/\bINT\s+PRIMARY\s+KEY\s+AUTOINCREMENT\b/gi, "INTEGER PRIMARY KEY AUTOINCREMENT");
    ...
    if (query.toUpperCase().startsWith("CREATE DATABASE") || query.toUpperCase().startsWith("DROP DATABASE")) {
        return `-- DATABASE commands are not supported in SQLite.`;
    }
    if (query.toUpperCase().startsWith("ALTER TABLE") && query.toUpperCase().includes("ADD FOREIGN KEY")) {
